...
#TO-DO
#vec = c(0,0,1,2)
#values = sample(vec, size = n*n, replace = TRUE, prob = NULL)
values = c(rep(0,.5*n*n),rep(1,.25*n*n),rep(2,.25*n*n))

R = matrix(sample(values), ncol = n, nrow = n)
#R = matrix(round(runif(n*n,0,2),0), nrow = n, ncol= n)
table(R)
...
```{r}
#TO-DO
#populate vector with sums of rows/
#assign vector to rows using r.names
#order the matrix by row name
row_val = c()
for(i in 1:n){
...
#There are three different species of Irises. We are also provided with information from 'Sepal.length', 'Sepal.Width', 'Petal.Length', 'Petal.Width'; which provide us with information about the sepal and petal length and width. Providing us statistically usefull information such at min, the quariles, max.
```
