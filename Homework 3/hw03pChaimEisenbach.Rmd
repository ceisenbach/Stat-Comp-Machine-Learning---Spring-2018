...
The intercept is color D, cut Fair, clarity I1. 
**TO-DO
...
#TO-DO
#diamonds1 = diamonds
#diamonds1$fair = ifelse(diamonds1$cut =="Fair", 1,0)
#diamonds1$good = ifelse(diamonds1$cut =="Good", 1,0)
#diamonds1$very_good = ifelse(diamonds1$cut =="Very Good", 1,0)
#diamonds1$premium = ifelse(diamonds1$cut =="Premium", 1,0)
#diamonds1$ideal = ifelse(diamonds1$cut =="Ideal", 1,0)
#diamonds1$d = ifelse(diamonds1$color =="D", 1,0)
#diamonds1$e = ifelse(diamonds1$color =="E", 1,0)
#diamonds1$f = ifelse(diamonds1$color =="F", 1,0)
#diamonds1$h = ifelse(diamonds1$color =="H", 1,0)
#diamonds1$i = ifelse(diamonds1$color =="I", 1,0)
#diamonds1$j = ifelse(diamonds1$color =="J", 1,0)
#diamonds1if = ifelse(diamonds1$clarity =="IF",1,0)
#diamonds1$vvs1 = ifelse(diamonds1$clarity =="VVS1", 1,0)
#diamonds1$vvs2 = ifelse(diamonds1$clarity =="VVS2", 1,0)
#diamonds1$vs1 = ifelse(diamonds1$clarity =="VS1", 1,0)
#diamonds1$vs2 = ifelse(diamonds1$clarity =="VS2", 1,0)
#diamonds1$si1 = ifelse(diamonds1$clarity =="SI1", 1,0)
#diamonds1$si2 = ifelse(diamonds1$clarity =="SI2", 1,0)
#diamonds1$cut = NULL
#diamonds1$color = NULL
#diamonds1$clarity = NULL
diamonds2 = diamonds
diamonds2$cut = factor(as.numeric(diamonds$cut))
diamonds2$clarity = factor(as.numeric(diamonds$clarity))
diamonds2$color = factor(as.numeric(diamonds$color))
mod7 = lm(price ~ ., diamonds2)
coef(mod7)
...
X2 = as.matrix(cbind(1, diamonds2))

t(X2) %*% X2
XtX = t(X1) %*% X1
XtXinv = solve(XtX, tol = 1e-22)
H = X1 %*% XtXinv %*% t(X1)
yhat = H %*% y
head(yhat)
```
...
```{r}
#TO-DO
mod5 = lm(price ~ cut+color+clarity+poly(carat, 5, raw = TRUE)+poly(x, 5, raw = TRUE)+poly(y, 5, raw = TRUE)+poly(z, 5, raw = TRUE)+poly(depth, 5, raw = TRUE)+poly(table, 5, raw = TRUE), diamonds)
```
...
an error in the thousands is pretty significant so we probably dont want this model.
**TO-DO
