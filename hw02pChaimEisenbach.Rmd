...
The line is too close to our values. We would rather have a line "in middle" of both sets of values.
...
It looks better. It seperates the data more evenly, as it's seemingly equidistant from the two sets of data.
TO-DO
...
nn_algorithm_predict = function(Xinput, y_binary, Xtest){ #first row, dot with second row
  #TO-DO
  best_sqd_distance = 1000 #good place to begin
  i_star = NA
  for (i in 1 : nrow(Xinput)){
...
$b_0$ - The regression seems to say that if the parents height is 0 the childs height would be 46 inches.
$b_1$ - The slope of the model is 0.3256475.
$R^2$ - The parents height only accounts for 21% of the childs height.
RMSE - The error between the two data sets is off by around 3 inches in 95% of our predictions.
